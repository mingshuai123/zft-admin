<template>
  <div class="mytask-container">
    <div class="page-title">
      <div class="left">
        提现详情 /
        <span>提现单ID : {{orderInfo.orderId}}</span>
      </div>
    </div>
    <div class="mytaskitem">
      <el-row class="row">
        <el-col :span="8">
          <el-tag class="name">提现单ID</el-tag>
      ...
      <el-row class="row">
        <el-col :span="8">
          <el-tag class="name">提现方式</el-tag>
          <span>{{orderInfo.type}}</span>
        </el-col>
        <el-col :span="8">
          <el-tag class="name">提现账号</el-tag>
          <span>{{orderInfo.accountId}}</span>
        </el-col>
        <el-col :span="8">
          <el-tag class="name">备注</el-tag>
          <span>{{orderInfo.memo}}</span>
        </el-col>
      </el-row>
    </div>
    <div>
      <el-button type="primary" @click="dialogVisible = true">已打款</el-button>
...
export default {
  ...
  methods: {
    handleClick(type) {
      ...
      var _this = this;
      this.postAxios2("withdraw/handler", {}, data).then(res => {
        if (res.code == 0) {
    ...
    getList() {
      ...
      this.postAxios("withdraw/info", data).then(res => {
